"""
CEB-Net data loading — compatible with CCFRec data format.
Ignores session_ids field.
"""
import os
import numpy as np
import torch
from utils import load_json, load_jsonl, get_seqs_len
...
class CEBNetDataset(Dataset):
    def __init__(self, args, n_items, inter_seq, index, mode="train"):
        ...
        self.inter_data = self._map_inter(inter_seq)

    def _map_inter(self, inter_seq):
        inter_data = []
        for seq in inter_seq:
            ...
            for item in item_seq:
                ...
                for i in range(self.n_digits):
                    ids.append(self.index[item][i] + i * self.args.n_codes_per_lel + 1)
                code_seq.extend(ids)
            inter_data.append({
                "item_inter": item_seq,
                "code_inter": code_seq,
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            L = len(item_inter)
            code_inter, mask_target = self._mask(code_inter.reshape(L, -1))

        return item_inter, target, code_inter, mask_target

    def _mask(self, code_inter):
        BL, C = code_inter.shape
        mask_target = np.ones_like(code_inter) * -100
        ...
        return code_inter.reshape(-1), mask_target.reshape(-1)
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        item_inters = pad_sequence([torch.tensor(x) for x in item_inters]).transpose(0, 1)
        code_inters = pad_sequence([torch.tensor(x) for x in code_inters]).transpose(0, 1)
        code_inters = code_inters.reshape(-1, self.n_digits)
        targets = torch.tensor(targets)
        mask_targets = pad_sequence(
            [torch.tensor(x) for x in mask_targets], padding_value=-100
        ).transpose(0, 1).reshape(-1)

        return dict(
            item_inters=item_inters.long(),
