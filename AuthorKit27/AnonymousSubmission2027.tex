...
Let $\mathcal{U}=\{u_1,\cdots,u_{|\mathcal U|}\}$ and $\mathcal{V}=\{v_1,\cdots,v_{|\mathcal V|}\}$ denote the user set and the item set, respectively. Each item $v\in\mathcal V$ is associated with a unique item ID as well as a set of textual attributes, such as the product title, brand, and category. To leverage semantic information, we employ a semantic encoder to obtain the semantic representation of each item, denoted by $\mathbf{z}_i\in\mathbb{R}^{d}$, where $d$ is the embedding dimension.

For each user, the historical interaction sequence is represented as $\mathcal{S}=\{v_1,\cdots,v_L\}$, and the corresponding sequence of semantic representations is denoted by $\mathbf{Z}=[\mathbf{z}_1,\cdots,\mathbf{z}_L]$. Given the historical interaction sequence $\mathcal{S}$, the objective of sequential recommendation is to predict the next item that the user is most likely to interact with, i.e., maximizing the conditional probability:$P(v_{L+1}\mid\mathcal S).$
\begin{figure*}[t]  % <--- 注意这里变成了 figure*，[t] 表示优先放页顶
  \centering        % 建议加上居中，否则图片可能偏左
  \includegraphics[width=\textwidth]{fracwork} % 这里 width=\textwidth 代表整个页面的宽度
...
Given the user behavior representation sequence: $\mathbf{Z}=[\mathbf{z}_1,\mathbf{z}_2,\cdots,\mathbf{z}_L],$

WEBD performs multi-scale decomposition using the Discrete Wavelet Transform (DWT):

$$
\mathbf{A}, \mathbf{D} = \text{DWT}(\mathbf{Z}).
$$

Here, $\mathbf{A}$ and $\mathbf{D}$ denote the low-frequency trend and high-frequency detail components, respectively. The low-frequency component captures stable interest evolution, while the high-frequency component reflects local behavioral changes. Since high-frequency variations may represent either noise or genuine bursty interests, adaptive discrimination is required.

\subsubsection{Adaptive Burst-preserving Denoising}

Conventional wavelet denoising relies on fixed thresholds, which cannot adapt to diverse behavioral patterns. Therefore, we learn context-dependent thresholds from the entire behavior sequence. Specifically, a global contextual representation is obtained by average pooling:

$$
\mathbf{c} = \frac{1}{L}\sum_{j=1}^{L}\mathbf{z}_j.
$$
...
$$
\boldsymbol{\tau} = \alpha \cdot \sigma\left(\text{MLP}\!\left(\mathbf{c}\right)\right),
$$

where $\alpha$ is a learnable scaling factor and $\sigma(\cdot)$ denotes the sigmoid function.

Given the learned thresholds, soft-thresholding is applied to the high-frequency coefficients:

$$
\mathbf{D}' = \text{sign}(\mathbf{D}) \cdot \max(|\mathbf{D}|-\boldsymbol{\tau},0).
$$

The adaptive threshold mechanism adjusts the denoising strength according to behavioral contexts. Specifically, smaller thresholds preserve consistent high-frequency patterns corresponding to bursty interests, while larger thresholds suppress irregular fluctuations caused by noise.
...
The reconstructed sequence is represented as: $\mathbf{H}=[\mathbf{h}_1,\mathbf{h}_2,\cdots,\mathbf{h}_L],$

where $\mathbf{H}$ denotes the refined behavior representations. By selectively removing random fluctuations while preserving stable trends and bursty interests, WEBD provides cleaner inputs for subsequent semantic memory modeling.
...
Given the refined behavior sequence from WEBD,$\mathbf{H}=[\mathbf{h}_1,\mathbf{h}_2,\cdots,\mathbf{h}_L],$ we divide it into working memory and long-term memory:$\mathbf{H}_{wm}=[\mathbf{h}_{L-m+1},\cdots,\mathbf{h}_L],
\mathbf{H}_{long}=[\mathbf{h}_{1},\cdots,\mathbf{h}_{L-m}].$

The working memory captures recent user intentions, while the long-term memory models stable preference patterns. We aggregate working memory through attention pooling:
...
where $\mathbf{k}_{wm}$ serves as the short-term interest representation for subsequent memory retrieval.

\subsubsection{Semantic Prototype Learning}

Directly retaining all historical behaviors introduces redundancy and makes it difficult to form reusable long-term memories. Therefore, we compress long-term behaviors into a set of semantic prototypes. First, positional embeddings and a Transformer encoder are applied to capture temporal dependencies:
$$
\widetilde{\mathbf{H}}_{long}=\text{Transformer}(\text{LN}(\mathbf{H}_{long}+\mathbf{P}_{long})).
$$

We maintain $K$ learnable semantic prototypes:

$$
\mathbf{P}=[\mathbf{p}_1,\ldots,\mathbf{p}_K]\in\mathbb{R}^{K\times d}.
$$

For each contextualized behavior representation $\widetilde{\mathbf{h}}_j$, its assignment probability over prototypes is computed as:

$$
\alpha_{j,k}=
\frac{\exp(\widetilde{\mathbf{h}}_j^\top\mathbf{p}_k/\tau_p)}
{\sum_{l=1}^{K}\exp(\widetilde{\mathbf{h}}_j^\top\mathbf{p}_l/\tau_p)}.
$$

The $k$-th semantic memory slot is obtained by weighted aggregation:

$$
\mathbf{m}_k=
\frac{\sum_j\alpha_{j,k}\widetilde{\mathbf{h}}_j}
{\sum_j\alpha_{j,k}+\epsilon}.
$$

The resulting long-term semantic memory is:

$$
\mathbf{M}_{sem}=[\mathbf{m}_1,\ldots,\mathbf{m}_K]\in\mathbb{R}^{K\times d}.
$$

By converting numerous historical interactions into a small number of semantic prototypes, the model obtains compact long-term memories that can be dynamically retrieved under different recommendation contexts.
...
To capture the current recommendation context, we construct a query from the complete behavioral trajectory rather than only recent interactions. However, semantic item representations may lack sufficient discrimination when different items share similar meanings. Therefore, we introduce an independent Trace ID embedding table in the retrieval branch:$\mathbf{E}^{trace\_id} \in \mathbb{R}^{|\mathcal{I}| \times d}.$ For item $i$, its Trace ID representation is obtained as: $\mathbf{e}_i=\mathbf{E}^{trace\_id}[i].$

We inject the Trace ID information into the denoised behavior representation through a gated residual mechanism:

$$
g_i = \sigma\!\left(\mathbf{W}_g[\mathbf{h}_i;\mathbf{e}_i]+b_g\right),
$$

$$
\hat{\mathbf{h}}_i^{trace}
=
\text{LN}(\mathbf{h}_i+g_i\odot\mathbf{e}_i),
$$

where $\mathbf{h}_i$ denotes the behavior representation produced by WEBD. The trajectory-enhanced sequence is formulated as:$\hat{\bm{H}}
=
[\hat{\bm{h}}_1,\hat{\bm{h}}_2,\cdots,\hat{\bm{h}}_L].$

After adding positional embeddings, a causal Transformer is applied:

$$
\mathbf{H}_{trace}
=
\text{CausalTransformer}
(\hat{\mathbf{H}}+\mathbf{P}_{trace}).
$$

The hidden state at the last position is used as the context-aware query: $\mathbf{q}_{trace}
=
\mathbf{H}_{trace}[L].$
...
Given the context-aware query, DMR retrieves relevant semantic memories from $\mathbf{M}_{sem}$. To avoid the conflict between retrieval-oriented matching and prediction-oriented representation learning, we introduce two independent spaces: the Retrieval Space and the Representation Space.

The query and semantic memories are first projected into the retrieval space:

$$
\mathbf{q}^{\mathcal{A}}
=
\mathbf{W}_{q}\mathbf{q}_{trace},
\mathbf{K}^{\mathcal{A}}
=
\mathbf{W}_{k}\mathbf{M}_{sem}.
$$

The retrieval weights are computed using scaled dot-product attention:
...
\frac{
\mathbf{K}^{\mathcal{A}}
(\mathbf{q}^{\mathcal{A}})^\top
}
{\sqrt{d_a}}
\right).
$$

The retrieval space only determines memory relevance. The retrieved semantic information is then mapped into an independent representation space:

$$
\mathbf{V}^{\mathcal{R}}
=
\mathbf{W}_{v}\mathbf{M}_{sem},
$$

and aggregated according to the retrieval weights:

$$
\mathbf{r}_{mem}
=
\boldsymbol{\beta}\mathbf{V}^{\mathcal{R}}.
$$

Finally, we fuse the current behavioral context with the retrieved semantic memory through gated residual learning:

$$
\mathbf{g}_{mem}
=
\sigma
(\mathbf{W}_g[\mathbf{q}_{trace};\mathbf{r}_{mem}]),
$$
...
\mathbf{q}_{trace}
+
\mathbf{g}_{mem}\odot\mathbf{r}_{mem}
).
$$

By decoupling retrieval and representation learning, DMR allows memory matching and semantic prediction to be optimized independently, producing a more expressive user representation while enabling adaptive coordination between short-term intentions and long-term preferences.
...
Given the learned user representation $\mathbf{z}_u$, we adopt sampled softmax cross-entropy for next-item prediction. Given the positive item $v^{+}$ and sampled negative items $\mathcal{N}$, the recommendation loss is defined as:
...
\langle
\hat{\mathbf z}_u,
\hat{\tilde{\mathbf z}}_j
\rangle/\tau)
},
$$

and $\mathcal{L}_{\tilde{u}\rightarrow u}$ is defined symmetrically. Here,
$\hat{\mathbf z}=\mathbf z/\|\mathbf z\|_2$ denotes the normalized representation and $\mathcal{B}$ represents the mini-batch user set.
...
where $\lambda_{CL}$, $\lambda_{proto}$, and $\lambda_{freq}$ balance the contributions of the auxiliary objectives.
...
\begin{table*}[t]
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l cccc cccc cccc}
    ...
    % --- Row 0: DHPRec (Full) ---
    (0) DHPRec &
    \textbf{0.0315} & \textbf{0.0504} & \textbf{0.0204} & \textbf{0.0265} &
    \textbf{0.0703} & \textbf{0.1114} & \textbf{0.0442} & \textbf{0.0578} &
    \textbf{0.0452} & \textbf{0.0712} & \textbf{0.0290} & \textbf{0.0370} \\
    % --- Row 1: w/o FDFE ---
    (1) w/o FDFE &
    0.0302 & 0.0484 & 0.0193 & 0.0252 &
    0.0675 & 0.1068 & 0.0427 & 0.0554 &
    \underline{0.0441} & \underline{0.0705} & \underline{0.0285} & 0.0367 \\
    % --- Row 2: w/o Pattern ---
    (2) w/o Pattern &
    0.0308 & 0.0490 & 0.0196 & 0.0255 &
    \underline{0.0682} & \underline{0.1091} & \underline{0.0432} & \underline{0.0562} &
    0.0437 & 0.0701 & 0.0284 & \underline{0.0369} \\
    % --- Row 3: Fusion-Concat ---
    (3) Fusion-Concat &
    \underline{0.0309} & \underline{0.0491} & \underline{0.0199} & \underline{0.0257} &
    0.0677 & 0.1073 & 0.0429 & 0.0559 &
    0.0436 & \underline{0.0705} & 0.0281 & 0.0367 \\
    % --- Row 2: w/o Pattern ---
    (4) w/o query &
    0.0308 & 0.0490 & 0.0196 & 0.0255 &
    \underline{0.0682} & \underline{0.1091} & \underline{0.0432} & \underline{0.0562} &
